...
bool removeKey(key_value key) {
    ...
    // this is by design so we don't orphan existing keys down the chain
    return true;
}
...
bool setInt(std::string key, int value) {
    ...
    key_value kv;
    kv.setKey(key);
    kv.type = getValueTypeByName("int64");
    ...
    for(int i=0; i<8; i++) {
        kv.value[i] = '\0';
        kv.value[i] |= (uint8_t)((val & mask) >> offset);
        mask >>= 8;
        offset -= 8;
    }
